...
def aggregate(model, clients, lengths):
    ...
    for i in global_state_dict.keys():
        client_dicts [clients[k].state_dict()[i].float()*(num_selected*client_lens[k]/dataset_size) for k in range(num_selected)]
...
for r in range(num_rounds):
    ...
    for i in tqdm(range(num_selected)):
        load_model(client_models[i], glob_model)
        loss += update(client_models[i], opt[i], train_loader[client_index[i]], epochs)
        loss_retrain += update(client_models[i], opt[i], loader_fixed, retrain_epochs)
        updates += 150
    ...
    aggregate(glob_model, client_models,client_lens) # Model aggregation
    test_loss, acc = test(glob_model, test_loader)
    loss_test.append(test_loss)
    acc_test.append(acc)

    print(f'Round {r+1}')
    print(f'Train Loss: {loss_retrain_list[-1]} | Test Accuracy: {acc} | Test Loss: {test_loss} | Update: {updates}')
